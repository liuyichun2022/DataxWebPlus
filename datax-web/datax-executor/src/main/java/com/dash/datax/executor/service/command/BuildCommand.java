package com.dash.datax.executor.service.command;

import com.dash.datax.executor.service.jobhandler.DataXConstant;
import com.dash.datax.executor.util.SystemUtils;
...
public class BuildCommand {
    ...
    private static String buildJVMParam(TriggerParam tgParam) {
        ...
        if (StringUtils.isNotBlank(jvmParam)) {
            doc.append(DataXConstant.JVM_CM).append(DataXConstant.TRANSFORM_QUOTES).append(jvmParam.replaceAll(DataXConstant.SPLIT_SPACE, DataXConstant.TRANSFORM_SPLIT_SPACE)).append(DataXConstant.TRANSFORM_QUOTES);
        }
        return doc.toString();
    }
    ...
    public static HashMap<String, String> buildDataXParamToMap(TriggerParam tgParam) {
        ...
        if (IncrementTypeEnum.partition.contains(incrementType)) {
            ...
            if (StringUtils.isNotBlank(partitionStr)) {
                ...
                String formatParam = String.format(DataXConstant.PARAMS_CM_V_PT, buildPartition(partitionInfo));
                return getKeyValue(formatParam);
            }
    ...
    private static HashMap<String, String> getKeyValue(String formatParam) {
        String[] paramArr = formatParam.split(DataXConstant.PARAMS_SYSTEM);
        HashMap<String, String> map = new HashMap<>();

        for (String param : paramArr) {
            if (StringUtils.isNotBlank(param)) {
                param = param.trim();
                String[] keyValue = param.split(DataXConstant.PARAMS_EQUALS);
