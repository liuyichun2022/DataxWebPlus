...
import com.daas.datax.executor.service.command.BuildCommand;
...
import com.pji.cloud.datatx.core.thread.ProcessCallbackThread;
import com.pji.cloud.datatx.core.util.ProcessUtil;
import com.daas.datax.executor.service.logparse.LogStatistics;
...
import static com.daas.datax.executor.service.jobhandler.DataXConstant.DEFAULT_JSON;
import static com.daas.datax.executor.service.logparse.AnalysisStatistics.analysisStatisticsLog;
...
public class ExecutorJobHandler extends AbstractJobHandler {
    ...
    @Value("${datax.pypath:/data/datax/datax/bin}")
    private String dataXPyPath;

    @Value("${datax.executor.pythonpath:/data/datax/Python-2.7.15}")
    ...
    public ReturnT<String> execute(TriggerParam trigger) {
        ...
        HashMap<String, String> keyValueMap = BuildCommand.buildDataXParamToMap(trigger);
        String jobJson = replaceVariable(trigger.getJobJson(), keyValueMap);
        Map<String, String> buildin = BuildCommand.builtInVar();
        ...
        try {
            //检查运行环境
            CheckEnv.checkEnv(dataXPyPath,pythonPath);
            String[] cmdarrayFinal = BuildCommand.buildDataXExecutorCmd(trigger, tmpFilePath, dataXPyPath, pythonPath);
